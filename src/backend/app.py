...
engine = create_engine(f"sqlite+libsql://medicaldb-dnair.turso.io?authToken={AUTH_TOKEN}")
...
def register_user():
    data = request.json
    query = text("""
        INSERT INTO Usuario (Rut, digito_verificador, nombre, apellido, fec_nac, direccion, cod_comuna, email, telefono, cod_tipo_user, cod_esp, password)
        VALUES (:rutNum, :rutDig, :name, :surname, :birthday, :direccion, :comuna, :email, :phone, 1, Null, :password)
    """)
    try:
...
def login_cliente():
    ...
    try:
        with engine.connect() as connection:
            sql = text("SELECT * FROM clientes WHERE rut = :rut")
            result = connection.execute(sql, {'rut': data["rutNum"]}).fetchone()
        ...
        return jsonify({'message': 'Ocurrió un error: ' + str(e)}), 500
